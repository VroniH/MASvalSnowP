...
def diffHS(fRR):
	...
	SMET_data = pd.DataFrame.from_dict({
		...
		'PSUM':ERA5['RRmm']*fRR.item(),
		})
		# 'PSUM':(eklima['RR']*fRR.item()).round(2)})
	...
	create_SMET(names[i],experiment_name+'_'+str(int(fRR.item()*10)),ID[i],latitude[i],longitude[i],easting[i],northing[i],heights[i],SMET_data,path_in)
	create_SNO(names[i],experiment_name+'_'+str(int(fRR.item()*10)),ID[i],latitude[i],longitude[i],easting[i],northing[i],heights[i],slope[i],azi[i], path_in)
	INI_file = create_INI(names[i],experiment_name+'_'+str(int(fRR.item()*10)), path_ini,path_in,path_out)

	# Remove output files--------------------------------------------------
	# remove previously built output files, so memory is not swampt with unnecessary files 
	# for files in os.listdir('/home/user/Desktop/Masterarbeit/SnowPACK/Masterarbeit/output_optimization/search/'):
	# 	os.remove('/home/user/Desktop/Masterarbeit/SnowPACK/Masterarbeit/output_optimization/search/' + files)
	# RUN SNOWPACK -------------------------------------------------
	...
	files = []
	for file in os.listdir('/home/user/Desktop/Masterarbeit/SnowPACK/Masterarbeit/output_optimization/search/'): # list files in output directory
		if file.endswith(".pro"): 
			files.append(file)  # get list of .pro files
	paths = [os.path.join('/home/user/Desktop/Masterarbeit/SnowPACK/Masterarbeit/output_optimization/search/', basename) for basename in files] # create file path
			...
			with open(latest,'r') as file:	
				...
				for l, line in enumerate(file):		# read lines
					...
					if '0500,'+ Date == line:			# finde right entry, according to date of manual snowpit
						HS_mod = next(file)			# jump to next line
						HS_mod = HS_mod.split(',')[-1]	# split entries by sep = , save last value (-1) in line
						# print(l, line, HS_mod)
						# print(HS_man,float(HS_mod))
						diff_HS = abs(float(HS_man) - float(HS_mod))	# write difference manual - modelled HS to variable
						x.append(fRR.item())							# write list of rain factors
...
# i = 3									# Try for single pits
	...
	x=[]
	f_x=[]
	opt = optimize.brute(diffHS, ((0.5,2.5),), args=(), Ns=21,
	...
	res = open('/home/user/Desktop/Masterarbeit/SnowPACK/Masterarbeit/'+names[i]+experiment_name+'.txt', mode = 'a')
